...
//                        .option(ChannelOption.ALLOW_HALF_CLOSURE, true)
    ...
    public int send(YCRequest request) {
        ...
        mRequestTable.put(sn, request);
        send2Net(request);
        if (mState == STATE_HANDSHAKEED) {
            //向网络发送请求

        }
        mTimeOutTable.put(sn, request.mTimeout);
        return sn;
    }
...
//        }
     ...
     * 网络协议编码
     */
    public class NetPackageEncoder extends MessageToByteEncoder<NetPackage> {

        @Override
        protected void encode(ChannelHandlerContext arg0, NetPackage arg1, ByteBuf arg2)
                throws Exception {
